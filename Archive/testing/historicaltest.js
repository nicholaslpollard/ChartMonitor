...
async function loadProgress() {
  ...
  return { completed: {}, invalidSymbols: [] };
}
...
async function fetchOHLCV(symbol, timeframe, from, to) {
  try {
    ...
    if (err.response && err.response.status === 400) {
      console.warn(`⚠️ Invalid symbol detected: ${symbol}`);
    } else {
      console.warn(`⚠️ Failed API fetch for ${symbol} ${timeframe}: ${err.message}`);
    }
    return null; // return null to indicate invalid symbol
  }
}
...
async function runDownloadQueue(tasks, progress) {
  return new Promise((resolve) => {
    ...
    const startNext = async () => {
      ...
      if (data === null) {
        if (!progress.invalidSymbols.includes(symbol)) progress.invalidSymbols.push(symbol);
        activeDownloads--;
        startNext();
        return;
      }
...
async function main() {
  ...
  let csvData = await fs.readFile(STOCKS_FILE, 'utf8');
  let symbols = csvData.split(/\r?\n/).map(s => s.trim()).filter(Boolean);

  // Remove previously detected invalid symbols
  if (progress.invalidSymbols && progress.invalidSymbols.length > 0) {
    symbols = symbols.filter(s => !progress.invalidSymbols.includes(s));
  }

  // Ensure SPY is always included
  if (!symbols.some(s => s.toUpperCase() === 'SPY')) symbols.unshift('SPY');
  ...
  for (const rawSymbol of symbols) {
    const symbol = rawSymbol.trim().split(',')[0].replace(/"/g, '');
